...
export default function App() {
  ...
  // States for Goal and Events loaded from backend File Store
  ...
  async function handleSaveGoal(title: string, targetMonth: number, targetDay: number) {
    try {
      ...
      console.error('Failed to save goal to server', err);
    }
  }
  ...
  async function handleToggleCrossed(dateKey: string) {
    ...
    try {
      ...
      console.error('Failed to save crossed status', err);
    }
  }
    ...
    <div dir="rtl" className="min-h-screen bg-stone-50 text-stone-900 flex flex-col font-sans antialiased select-none">
      ...
      <header className="bg-white border-b border-stone-200 py-3.5">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-2 text-xs text-stone-500 bg-stone-50 border border-stone-150 px-3 py-1.5 rounded-lg font-sans">
            <span className="h-2 w-2 rounded-full bg-emerald-500 inline-block animate-pulse"></span>
            <span>میز کاری متصل به فایلdata-store.json</span>
          </div>
        </div>
      </header>
